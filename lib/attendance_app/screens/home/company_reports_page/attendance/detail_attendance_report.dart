...
class DetailAttendanceReport extends StatefulWidget {
  const DetailAttendanceReport({super.key});

  @override
  State<DetailAttendanceReport> createState() => _DetailAttendanceReportState();
}

class _DetailAttendanceReportState extends State<DetailAttendanceReport> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: AppColors.white,
        title: Text("Detail Attendance Report",
      ...
      body: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text("Select Department").marginAll(h10),
            //TODO All Branches
            ListTile(
              onTap: (){
                showModalBottomSheet(
                    ...
                    builder: (BuildContext context) {
                      return SizedBox(
                        ...
                        child: Column(
                          children: [
                            Text("Select Department", style: AppTextStyles.kCaption12WRegularTextStyle,).marginAll(h20),
                          ],
                        ),
                      );
                    });
              ...
              shape: RoundedRectangleBorder(
                side: const BorderSide(color: AppColors.white60, width: 1),
                borderRadius: BorderRadius.circular(5),),
              title: const Text("All Department"),
              trailing: const Icon(Icons.keyboard_arrow_right_outlined),
            ).paddingOnly(left: h10, right: h10, bottom: h15),

            Obx(() => ListTile(
              leading: const Icon(Icons.error, size: 20,),
              title: Text("Payroll Attendance Cycle", style: AppTextStyles.kSmall8RegularTextStyle,),
              trailing: CupertinoSwitch(value: radioController.isShow.value,
                  onChanged: (value){
            ...
            Obx(() =>CustomRadioButton(
                options: [
                  RadioOption(value: 1, title: "PDF"),
                  RadioOption(value: 2, title: "CSV"),
                ],
                onChanged: (value, title){
